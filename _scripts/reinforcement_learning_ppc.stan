data {
  ...
  real<lower=-1, upper=1>  outcome[N, T];  // no lower and upper bounds
}
transformed data {
...
transformed parameters {
  ...
  A   = Phi_approx(mu_p[1] + sigma[1]  * A_pr);
  tau = Phi_approx(mu_p[2] + sigma[2] * tau_pr) * 5;
}
model {
  // Hyperparameters
  mu_p  ~ normal(0, 1);
  ...
  { // local section, this saves time and space
    for (i in 1:N) {
      ...
      for (t in 1:(Tsubj[i])) {
        ...
        y_pred[i, t] = categorical_logit_rng(tau[i] * ev);
